...
class FFmpeg {
    ...
    async sliceVideo(startSec: number, endSec: number, fileName: string, outFileName: string) {
        await this.ffmpeg.run("-ss", startSec.toString(), '-t', (endSec - startSec).toString(), '-i', fileName, '-c', 'copy', outFileName);
    }
